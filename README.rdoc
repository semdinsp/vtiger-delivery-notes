...
== Variables
product list - a list of commma separated values starting from 1 which selects the products on the sales order - so that you can have separate delivery notes for a sales order
customer reference - a reference value that customers can use ( for example their PO (purchase order) number)
remarkds - the remarks portion comes from the description field on the sales order
